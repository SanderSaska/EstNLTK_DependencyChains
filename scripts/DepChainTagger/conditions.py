...
# TODO: Add an option to condition pattern more dynamically by giving attribute name and its value. For example, instead of having separate upostag_condition, xpostag_condition, lemma_condition, etc., we could have a more generic attribute_conditions: Dict[str, ValueCondition] that allows specifying conditions for any attribute by name. This would make the NodeConstraint more flexible and extensible without needing to add new fields for each attribute type.

# TODO: EdgeConstraint has a bug, where if we try to replicate the syntax relation in reverse order (e.g., from parent to child instead of child to parent), we do not get the match.
